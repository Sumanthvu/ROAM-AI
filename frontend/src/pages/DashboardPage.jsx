...
//     setLoading(true);
//     setError('');
//     const payload = { ...preferences, total_budget: parseInt(preferences.total_budget, 10), no_of_people: parseInt(preferences.no_of_people, 10), duration: parseInt(preferences.duration, 10), budget_range: null };
//     try {
...
//       }
//     } catch (err) {
//       let specificError = 'Failed to fetch suggestions. Please check input and try again.';
...
//     const requestBody = { preferences: { ...preferences, total_budget: parseInt(preferences.total_budget, 10), no_of_people: parseInt(preferences.no_of_people, 10), duration: parseInt(preferences.duration, 10) }, selected_place: place.place };
//     try {
...
//     setLoading(true);
//     setError('');
//     const basePreferences = { ...preferences, total_budget: parseInt(preferences.total_budget, 10), no_of_people: parseInt(preferences.no_of_people, 10), duration: parseInt(preferences.duration, 10) };
//     const scheduleBody = { preferences: basePreferences, selected_place: selectedPlace.place, selected_attractions: selectedAttractions, selected_cuisines: selectedCuisines };
//     const localInfoBody = { preferences: basePreferences, selected_place: selectedPlace.place };
//     const budgetBody = { preferences: basePreferences };
//     try {
//       const [scheduleRes, safetyRes, packingRes, budgetRes, reviewsRes, transportRes, accommodationRes] = await Promise.all([ axiosMlClient.post('/schedule-trip', scheduleBody), axiosMlClient.post('/safety-info', localInfoBody), axiosMlClient.post('/packing-list', localInfoBody), axiosMlClient.post('/budget-breakdown', budgetBody), axiosMlClient.post('/reviews', localInfoBody), axiosMlClient.post('/transport-options', localInfoBody), axiosMlClient.post('/accommodation-suggestions', localInfoBody) ]);
//       const comprehensiveItinerary = { itinerary: scheduleRes.data.formatted.itinerary, safety: safetyRes.data.formatted, packing: packingRes.data.formatted, budget: budgetRes.data.formatted, reviews: reviewsRes.data.formatted, transport: transportRes.data.formatted, accommodation: accommodationRes.data.formatted };
